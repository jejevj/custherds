...
export default function GuideLoginPage() {
  ...
  // Redirect kalau sudah login sebagai guide
  if (hasHydrated && isLoggedIn && user?.user_type === 1) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.user?.user_type !== 1) {
        setError('Akun ini bukan akun Guide. Gunakan halaman login yang sesuai.')
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-emerald-950 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-emerald-400 text-sm mt-1">Guide Portal</p>
        </div>
        <Card className="border-slate-700 bg-slate-800/50 backdrop-blur">
          <CardHeader className="pb-4">
            <CardTitle className="text-white text-xl">Masuk sebagai Guide</CardTitle>
            <CardDescription className="text-slate-400">Login dengan akun Guide Anda</CardDescription>
          </CardHeader>
          <CardContent>
