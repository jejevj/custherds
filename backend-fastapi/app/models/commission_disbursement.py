...
class CommissionDisbursement(Base):
    """
    Batch pembayaran komisi ke guide.
    Dijadwalkan periodik (mingguan/bulanan) oleh superadmin atau sistem.
    ...
    # Snapshot data bank guide saat disbursement
...
class CommissionDisbursementItem(Base):
    """
    Detail transaksi yang masuk dalam satu batch disbursement.
    """
    ...
    guide_commission    = Column(Numeric(15, 2), nullable=False)  # snapshot nominal dari transaction
    created_at          = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
