...
class Booking(Base):
    """
    Reservasi yang dibuat oleh Guide untuk membawa turis ke vendor.

    Status flow:
        pending_vendor -> approved -> completed
                       -> cancelled

    Tourist tidak memiliki akun — data turis dicatat sebagai teks saja.
    """
    __tablename__ = "bookings"

    id                      = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    booking_code            = Column(String(20), unique=True, nullable=False, index=True)  # BK-YYYYMMDD-XXXX
    ...
    tourist_names           = Column(Text, nullable=True)   # plain text atau JSON array nama-nama turis
    ...
    # pending_vendor | approved | completed | cancelled
